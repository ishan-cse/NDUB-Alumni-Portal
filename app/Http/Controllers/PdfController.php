...
class PdfController extends Controller{
   ...
   public function adminAllSelectedEntryPassPDF(Request $request){
        ...
        // Set the maximum number of records to be fetched in chunks (adjust this number to suit your needs)
        $maxRecords = 1000;
        
        // Retrieve the program and batch info
        $program = Program::where('program_id', $program_id)->first(); // Assuming you have a Program model
        $batch = Batch::where('batch_id', $batch_id)->first(); // Assuming you have a Batch model
        
        // Check if program and batch exist
        if (!$program || !$batch) {
            // Handle the case where either program or batch does not exist
        ...
        // Loop through the records in chunks of 100
        for ($start = 0; $start < $maxRecords; $start += $perPage) {
            
            // Fetch the data in chunks
            $allData = GraduateList::where('registration_confirmation_status', 1)
                ->where('child_account_status', 0)
                ->where('program_id', $program_id)
                ->where('batch_id', $batch_id)
                ->with(['programInfo', 'batchInfo', 'secondProgramInfo'])
                ->skip($start)  // Skip the first $start records
                ->take($perPage) // Take $perPage records
                ->get();
            ...
            // Generate the PDF for the current chunk of records
            $pdf = Pdf::loadView('pdf.entry-pass', ['allData' => $allData])
                ->setOptions(['defaultFont' => 'sans-serif'])
                ->setPaper('A5');
            
            // Create the filename using program name, batch name, and current date
            $fileName = $batch->batch_name.' Batch - '.$program->program_name .' - Entry Pass - ' . $currentDate . '.pdf';
        
            // Download the PDF for the current chunk with the dynamic filename
